...
page.PageMenuToggler.prototype =
{
  ...
  onResize: function( event, ignoreDim )
  {
    ...
    if ( ignoreDim || (dim.width != this.windowDim.width || dim.height != this.windowDim.height) )
    {
      this.windowDim = dim;
...
page.FlyoutMenu.prototype =
{
  initialize: function( subMenuListItem )
  {
    ...
    if ( this.actionBar )
    {
      this.isMenuInActionBar = this.subMenuListItem.up('div.actionBar') ? true : false;
    ...
    if ( this.actionPanel )
    {
      this.isMenuInActionPanel = this.subMenuListItem.up('div.subActionBar') ? true : false;
    }
...
page.scrollToEnsureVisibleElement = function( )
{
  ...
  if ( receipt && receipt.visible() ) // pin receipt to top
  {
    ...
    offset.top = 0; 
    var w = parseInt(receipt.getStyle('width'));
    if ( Prototype.Browser.IE ) // width in IE includes border & padding, need to remove it
    {
      var bw = parseInt(receipt.getStyle('borderLeftWidth')) + parseInt(receipt.getStyle('borderRightWidth'));
      var pw = parseInt(receipt.getStyle('paddingLeft')) + parseInt(receipt.getStyle('paddingRight'));
      w = w - bw - pw;
    }
    receipt.setStyle({	position:"fixed", 
  			zIndex:"1000",
  			left: offset.left + "px", 
  			top: offset.top + "px", 
  			width: w + "px"});
