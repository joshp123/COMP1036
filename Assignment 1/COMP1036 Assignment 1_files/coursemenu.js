 ...
 */
  ...
  _showCourseMenuInWindow: function( event )
  {
    ...
    window.remote = window.open('/webapps/blackboard/content/courseMenu.jsp?course_id='+window.course_id+'&newWindow=true', 'newwin', 'width=220,height=440,resizable=yes,scrollbars=yes,status=no,top=20,left='+lpix);
    if ( window.remote )
    {
      window.remote.focus();
  ...
  _showDetailView: function( event )
  {
    ...
    if ( !this.tree || this.editMode )
    {
      ...
      this.tree = new dynamictree.Tree( this.detailViewContainer, null, '/webapps/blackboard/execute/course/menuFolderViewGenerator', 'course_id='+window.course_id+'&displayMode='+displayMode+'&editMode='+this.editMode, true );
    }
  ...
  {
    UserDataDWRFacade.getStringTempScope( 'courseMenuPalette_contents' + window.course_id, this.getCourseMenuPaletteStateResponse.bind( this ) );
