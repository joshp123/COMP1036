...
dynamictree.Tree.prototype =
{
   ...
   * @param retainState whether the state of node expand/collapse should be
   *        retained.  Default is false.
   * @param expandAllByDefault whether the full contents of the tree should be
   *        rendered/expanded on initial display.  Default is false.
   */
  initialize: function( container, initialTrees, generatorUrl, contextParameters, retainState, expandAllByDefault )
  {
  ...
  getLinkAndArea: function(toggler)
  {
    ...
    while (element && (!toggleLink || !toggleArea))
    {
      ...
      if (tagName == 'a') {
        ...
        toggler.folderName = element.getElementsByTagName('a')[1].innerHTML.strip();  // 2nd anchor is folder name
  ...
  collapseAll: function( )
  {
    if ( this.retainState )
    {
      var params = Object.extend({ collapseAll: "true", saveState: "true" }, this.contextParameters );
      new Ajax.Request( this.generatorUrl,
      {
        method: 'post',
        parameters: params,
